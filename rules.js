...
export function experienceNeeded(level){return Math.round(36+(level-1)*8);}
...
const heroSpell=(hero,id)=>EXTRA_BY_ID[id]?EXTRA_BY_ID[id].hero===(hero||'scout'):hero==='wraith'?!['fire','ice','storm'].includes(id):!['veil','chain','rift'].includes(id);
...
export function chooseUpgrades(p,random=Math.random){const pool=UPGRADES.filter(u=>heroSpell(p.heroId,u.id)&&(p.upgrades[u.id]||0)<u.max).map(u=>{
 ...
 return{...u,text};
 }),result=routeChoices(p);const fresh=pool.filter(u=>EXTRA_BY_ID[u.id]);if(fresh.length&&!EXTRA_SKILLS.some(u=>p.upgrades[u.id]>0)&&result.length<3){const choice=fresh[Math.floor(random()*fresh.length)];result.push(choice);pool.splice(pool.indexOf(choice),1);}while(pool.length&&result.length<3)result.push(pool.splice(Math.floor(random()*pool.length),1)[0]);return result;}
