...
export const WEAPONS={rifle:{id:'rifle',name:'游侠连发枪',rate:3,damage:12,count:1,speed:27,range:24,color:0xffdc91},shotgun:{id:'shotgun',name:'碎岩霰弹枪',rate:1,damage:10,count:5,speed:25,range:13,color:0xffbe69},fire:{id:'fire',name:'烬火法杖',rate:.9,damage:30,count:1,speed:14,range:22,color:0xff743b},pistol:{id:'pistol',name:'银鸦手枪',rate:2.2,damage:19,count:1,speed:32,range:25,color:0xffecc2},shuriken:{id:'shuriken',name:'月刃飞镖',rate:1.4,damage:13,count:3,speed:22,range:19,color:0x95fff0},dark:{id:'dark',name:'夜幕法器',rate:1,damage:26,count:1,speed:12,range:23,color:0xc5a2ff}};
export function weaponFor(hero,index){return WEAPONS[(hero==='silver'?['pistol','shuriken','dark']:['rifle','shotgun','fire'])[index]];}
...
export const WEAPON_PATHS={
 ...
 pistol_heavy:route('pistol','猎杀重弹','◆',['伤害 +30%，射速 -15%，射程 +15%','伤害 +60%，贯穿 2 个目标','伤害 +90%，贯穿 2 个目标']),
 pistol_ricochet:route('pistol','银鸦跳弹','↗',['弹向附近另一个目标 1 次，首发伤害 -5%，每跳保留 75% 伤害','最多跳弹 2 次','最多跳弹 3 次；同一发不重复命中']),
...
export function weaponStats(p){
 const base=WEAPONS[p.weaponId]||WEAPONS.pistol,w={...base,pierce:base.id==='shuriken'?2:1,spread:.14,radius:base.id==='fire'?2.5:base.id==='dark'?2:0,bounces:0,burn:0,returning:false,gravity:0};
 ...
 case'pistol_heavy':w.damage*=1+.3*r;w.rate*=.85;w.range*=1.15;w.pierce=r>=2?2:1;break;
 case'pistol_ricochet':w.bounces=r;w.damage*=.95;break;
