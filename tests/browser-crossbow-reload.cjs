...
(async()=>{
 ...
 try{
  ...
  const poses=await page.evaluate(async()=>{
   const {loadHeroAssets,createSkinnedHero,animateSkinnedHero,disposeHero}=await import('./skinned-hero.js?v=30');
   ...
   const rest=sample(1);d.shotSerial=1;const draw=sample(.5),returnPose=sample(1);disposeHero(hero);return {rest,draw,returnPose};
  });
