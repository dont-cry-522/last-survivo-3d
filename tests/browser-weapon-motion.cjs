...
(async()=>{
 ...
 try{
  ...
  const motions=await page.evaluate(async()=>{
   const {createSkinnedHero,animateSkinnedHero,disposeHero}=await import('./skinned-hero.js?v=13'),result={};
   for(const [kind,weapon] of [['scout','rifle'],['scout','shotgun'],['scout','fire'],['silver','shuriken'],['silver','dark']]){
    ...
    const neutral=[d.aimArm,d.firingForearm,d.offArm,d.offForearm].map(b=>b.quaternion.clone());
    d.reloadPhase=.35;animateSkinnedHero(hero,d.lastTime+1/60,0,.1,0);
    result[weapon]={arms:neutral.map((q,i)=>q.angleTo([d.aimArm,d.firingForearm,d.offArm,d.offForearm][i].quaternion)),pump:d.gun.userData.pump?.position.z||0};
    animateSkinnedHero(reference,rest.lastTime+1/60,0,0,0);
