...
(async()=>{const b=await chromium.launch({executablePath:process.env.BROWSER_EXECUTABLE||'C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe',headless:true});try{
 ...
 const results=await page.evaluate(async()=>{
  const {WEAPON_PATHS,takeUpgrade}=await import('./rules.js?v=5'),g=game3d,out=[];
  ...
  for(const [id,path]of Object.entries(WEAPON_PATHS)){
   const hero=['pistol','shuriken','dark'].includes(path.weapon)?'silver':'scout',index=(hero==='silver'?['pistol','shuriken','dark']:['rifle','shotgun','fire']).indexOf(path.weapon);
   setup(hero,index,id);const e=target(0,4);once();advance(.7);if(e.hp===e.maxHp)throw Error(id+' did not damage target');out.push(id);
  }
  setup('silver',0,'pistol_ricochet');const chain=[target(0,4),target(3,4),target(3,7)];once();advance(.8);if(!chain.every(e=>e.hp<e.maxHp))throw Error('Ricochet missed chain targets');
 ...
 assert.equal(results.length,12);assert.deepEqual(errors,[]);console.log('PASS all 12 routes damage targets; ricochet, return, burn, pull, pierce; VFX cap and cleanup');
 await page.close();
 for(const viewport of [{width:1440,height:900},{width:844,height:390},{width:390,height:844}]){
  ...
  await p.locator('[data-upgrade="path:pistol_ricochet"]').click();assert.equal(await p.evaluate(()=>game3d.player.weaponPath.id),'pistol_ricochet');
